...
1. **训练一次跑通，但评测推翻了 4 个结论** —— 重点不是犯错，而是**每次误诊都有对照实验支撑、每次推翻都留了记录**。两条可复用的方法论：**假基线陷阱**（strength=0 还被自造词污染）、**嫌疑清单按验证成本排序**。
2. **根因是最朴素的变量** —— 数据集/优化器/TE/精度全排除后，根因是画幅。靠的是**系统性排除法**，不是运气。
3. **训练环境被硬件卡死，五条铁律全部来自架构约束** —— fp16/SDPA/cuDNN/bitsandbytes 验证，每条都有实测的违反后果。
4. **放弃 GUI 走 CLI 是工程决策** —— 依赖体量、可复现性、可维护性三个维度，且产出物即交付物。
...
| 项 | 说明 |
| --- | --- |
| ~~SDXL LoRA~~ | ✅ **09-22 已完成**（见 §14 附录）：P100 16G 可训 |
