...
> **诚实结论**：这套硬件适合把 **SD 1.5 / SDXL 的工程链路跑通并做出可交付成果**，不适合追 FLUX 等新一代大模型。落地可行性才是关键——"能把平台搭起来、能排错、能交付"，比硬件规格本身更决定成败。
...
> 后续任何 `apt upgrade` 前，先 `apt-mark showhold` 确认锁定仍在。**这条本身就是一项运维能力，值得写进方案文档。**
...
掌握 ComfyUI 原生 API；写出**能上生产**的批量生成脚本；实现 Windows 发任务、Ubuntu 执行的双机算力调度；采集出第一组可用于容量规划的性能数据。
...
**Grafana 面板做不到"哪个进程吃了多少显存"的时间序列下钻**（dcgm 层面限制仍在），但**现场排查可以做**：`nvidia-smi --query-compute-apps=...` 或 `fuser -v /dev/nvidia*`。企业环境里 GPU 虚拟化很常见，**能说清"哪一层能拿到、哪一层拿不到"本身就是关键能力**。
...
四个核心指标（利用率/显存/温度/频率）两者都能给，**而 P100 拿不到的那些 DCP 指标恰恰是 dcgm 相对 nvidia-smi 的主要优势**。这台机器上 dcgm 的技术增量被架构限制吃掉了，但在方案文档里它更有分量。
...
## 阶段五：LLM 服务化 + 统一网关 + 交付整理（第 14~16 周）

### 阶段目标

补上 LLM 这一侧，用 FastAPI 做统一网关，整理交付物。
...
**`benchmarks/实测数据.md` 是容量规划与方案选型的数据源**，至少包含：
...
**有实测数字的容量规划，和凭经验的估算，是两个物种。**

### 5.5 能力覆盖

覆盖的技术面：GPU 运维、CUDA 生态与版本治理、容器化交付、Prometheus/Grafana 可观测性、
任务队列与 API 网关、LoRA 训练工程化、私有化部署方案、容量与成本测算。

**三个可复用的工程产出**：

1. **版本治理**：Blackwell 与 Pascal 的 CUDA 支持差异与驱动分支锁定策略（含 `verify_env.py` 驱动门禁）—— 这是真实踩过坑才有的知识
2. **可观测性**：GPU 利用率 / 显存 / 温度的 Grafana 面板，以及**按实测拐点标定**的告警规则
3. **量化数据**：所有性能数字都实测过，原始数据在 `benchmarks/raw/`，可用 `summarize_runs.py` 一键重算
...
4. 完整仓库交付（文档 + 脚本 + 配置 + 源码 + 原始数据）

---

## 附录 A：选修 —— K8s GPU 调度

> **优先级低**。单节点 16G + P100 上 K3s + GPU Operator 的投入产出比不高，且 GPU Operator 的驱动容器对 P100 支持存在风险。**建议放到主体交付完成之后再考虑，或只写一份方案设计而不实际搭建。**
...
| 工程价值 | "会用 GUI" | "能写参数化、可复现的训练脚本" |
| 工程化契合度 | 弱 | **强**（脚本化、自动化，可纳入 CI） |
...
4. ~~仓库交付整理~~ ✅ **09-20 完成**（`AIGC-Engineer-Portfolio/`：README（架构+实测+修正记录）+ docs 7 份 + scripts/gateway/docker/systemd/workflows/benchmarks + .gitignore），待 push
...
**动机**：SDXL 是更接近生产实际的模型代次；用 dog_lora_v2（v4 同款 20 张柴犬）同主体训练，与 SD1.5 v4 形成**同数据集跨代对照**。
...
- 进度：**阶段一 ✅ | 阶段二 ✅ | 阶段三 ✅ | 阶段四 ✅ | 阶段五 ✅ 全部完成（LLM 实测/网关生产化/三层监控/07 方案/交付整理）——16 周计划收官**
- 后续可选方向（按优先级排序）：**SDXL LoRA 🔵 训练中（D.15，明天评测）** → Grafana 应用层面板 ✅（09-21）→ 散热改造复测 ✅（09-21 双阴性，导风罩方案另排期）→ K8s 选修（附录 A）
...
- 下一步：可进阶段五（LLM 服务化 + 统一网关 + 交付整理），或先做 SDXL LoRA（P100 16G 可训）
- 剩余可选优化（不阻塞）：镜像再瘦身
