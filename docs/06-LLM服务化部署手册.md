...
1. **"我从 162 这个数字的结构反推出瓶颈"** —— 162 ≈ 32 tok ÷ 197ms/批，固定开销主导 → ubatch A/B 验证 → 零成本 3.2×。展示的是**假设驱动的调优**，不是瞎试参数。
2. **"Q4 比 Q8 慢，和社区常识相反"** —— Pascal 无 Tensor Core，反量化开销 > 带宽收益。展示**不把别人的结论当自己的结论**。
3. **"网关的每个设计决策都有实测依据"** —— 串行 worker、队列深度指标、异步/流式分流，全部能追到 benchmark 数字。
4. **"我推翻过自己平台限制的结论"** —— Proxmox 透传 per-process 归属，用户质疑后复测证伪，7 处文档同步修正。展示**证据优先于权威（包括自己的权威）**。
