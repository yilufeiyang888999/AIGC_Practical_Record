# 私有化 AIGC 平台：双机异构工程实践

> 一套**真实运行、全部数字实测**的私有化 AIGC 平台：双机异构部署 → API 自动化 → 容器化 → GPU 可观测性 → LoRA 训练 → LLM 服务化 → 统一网关 → 企业方案。
...
**主动声明边界比吹嘘能力更可信。** 这一节是本仓库的第一手资料——复用前请先读它。
...
在两台自有机器（RTX 5060 8G / Tesla P100 16G）上从零搭建可用的私有化生成平台，
覆盖部署、API 自动化、容器化、GPU 可观测性、LoRA 训练、LLM 服务化、统一网关全链路，
约束是**所有性能数字必须实测、可复现**。

之所以把过程完整留档，是因为这套硬件正处在 NVIDIA 生态的两端：
**Blackwell(sm_120) 与 Pascal(sm_60) 不存在任何共用的 PyTorch 安装包**，
几乎所有"照着官方教程装"的路径都会失败。踩过的坑和修正过程本身就是这个仓库的主要产出。

## 工程要点

1. **版本治理** —— Blackwell 与 Pascal 的 CUDA 支持差异、驱动分支锁定策略（`verify_env.py` 带驱动门禁）
2. **可观测性** —— GPU 利用率/显存/温度的 Grafana 面板 + **按实测拐点标定**的告警规则（79°C / 1200MHz）
3. **量化数据** —— 所有数字实测且原始数据可自验（`summarize_runs.py` 一键重算）

技术覆盖：**GPU 运维、CUDA 版本治理、容器化交付、Prometheus/Grafana 可观测性、任务队列与 API 网关、
LoRA 训练工程化、私有化部署方案、容量与成本测算。**
